...
$this->set('title', $data->opatreniNazev);
?>

<div id="tabs">
    <ul>
        <li><a href="#obecne">Obecné informace</a></li>
        <li><a href="#podopatreni">Podřízené MMR PodOpatření</a></li>
    ...
    <div id="podopatreni">
        <h2>Podřízené MMR PodOpatření</h2>
        <table class="datatable datatable_simple">
            ...
            <tbody>
            <?php foreach ($data->MmrPodOpatreni as $p) { ?>
                <tr>
                    <td><?= $this->Html->link($p->podOpatreniNazev, '/detail-mmr-podopatreni/?id=' . $p->idPodOpatreni) ?></td>
                    <td><?= $p->podOpatreniKod ?></td>
                    <td><?= $p->zaznamPlatnostOdDatum->year ?></td>
                    <td><?= $p->zaznamPlatnostDoDatum->year ?></td>
                    <td><?= $this->Html->link('Otevřít', '/detail-mmr-podopatreni/?id=' . $p->idPodOpatreni) ?></td>
                </tr>
            <?php } ?>
            </tbody>
            <tfoot>
