<?php
/**
  * @var \App\View\AppView $this
  */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Ciselnik Mestsky Obvod Mestska Castv01'), ['action' => 'edit', $ciselnikMestskyObvodMestskaCastv01->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Ciselnik Mestsky Obvod Mestska Castv01'), ['action' => 'delete', $ciselnikMestskyObvodMestskaCastv01->id], ['confirm' => __('Are you sure you want to delete # {0}?', $ciselnikMestskyObvodMestskaCastv01->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Ciselnik Mestsky Obvod Mestska Castv01'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Ciselnik Mestsky Obvod Mestska Castv01'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="ciselnikMestskyObvodMestskaCastv01 view large-9 medium-8 columns content">
    <h3><?= h($ciselnikMestskyObvodMestskaCastv01->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('MestskyObvodMestskaCastNazev') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->mestskyObvodMestskaCastNazev) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('ObecNad') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->obecNad) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Pad2') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->pad2) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Pad3') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->pad3) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Pad4') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->pad4) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Pad6') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->pad6) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Pad7') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->pad7) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('MestskyObvodMestskaCastKod') ?></th>
            <td><?= $this->Number->format($ciselnikMestskyObvodMestskaCastv01->mestskyObvodMestskaCastKod) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('GlobalniNavrhZmenaIdentifikator') ?></th>
            <td><?= $this->Number->format($ciselnikMestskyObvodMestskaCastv01->globalniNavrhZmenaIdentifikator) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('TransakceIdentifikator') ?></th>
            <td><?= $this->Number->format($ciselnikMestskyObvodMestskaCastv01->transakceIdentifikator) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('ZaznamPlatnostOdDatum') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->zaznamPlatnostOdDatum) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('ZaznamPlatnostDoDatum') ?></th>
            <td><?= h($ciselnikMestskyObvodMestskaCastv01->zaznamPlatnostDoDatum) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Pad5') ?></th>
            <td><?= $ciselnikMestskyObvodMestskaCastv01->pad5 ? __('Yes') : __('No'); ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('NespravnostIndikator') ?></th>
            <td><?= $ciselnikMestskyObvodMestskaCastv01->nespravnostIndikator ? __('Yes') : __('No'); ?></td>
        </tr>
    </table>
</div>
